import DarkButton from "@/components/UI/Buttons/DarkButton.vue"
import LoadingIcon from "@/components/UI/LoadingIcon.vue"
import TransparantButtonNB from "@/components/UI/Buttons/TransparantButtonNB.vue"
import NsudaInfo from "@/components/UI/NsudaInfo.vue"
import TransparentPlateWithCircle from "@/components/UI/TransparentPlateWithCircles.vue"
import BrightCheckbox from "@/components/UI/BrightCheckbox.vue"

export default [
    DarkButton,
    LoadingIcon,
    TransparantButtonNB,
    NsudaInfo,
    TransparentPlateWithCircle,
    BrightCheckbox
]